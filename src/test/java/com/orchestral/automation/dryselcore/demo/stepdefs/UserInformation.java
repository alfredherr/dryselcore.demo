...
import com.orchestral.automation.dryselcore.demo.userinfo.UserInfoInput;
import com.orchestral.automation.dryselcore.demo.userinfo.UserInfoOutput;
...
public class UserInformation {
	...
	public void loginAsJoe() {
		try {
			new StepDefHelper().getTestExecutor().executeInputScript(new UserInfoUI().getTestObjects(),
					new UserInfoInput().getTestObjects());
	...
	public void user_information_page_should_be_displayed() {
		try {
			new StepDefHelper().getTestExecutor().executeOutputScript(new UserInfoUI().getTestObjects(),
					new UserInfoOutput().getTestObjects());
		} catch (final ElementNotEnabledException e) {
			e.printStackTrace();
		}
	}
